...
const ws = new WebSocket("ws://192.168.1.104:8081");
...
ws.onmessage = (mensagemRecebida) => {
  ...
  if (resultado.conversa_id == conversa_id) {
    ...
    mensagemChat.insertAdjacentHTML(
      "beforeend",
      `<div  title="${resultado.nome}" class="flex ms-5  gap-1  justify-start shadow-lg">
       <p class="text-sm w-7 h-7 mt-1 leading-7 text-white bg-slate-700 text-center rounded-full ">${resultado.nome[0]}</p>
        <p class="flex flex-col max-w-[70%] bg-slate-700  text-justify  text-white justify-center p-2 my-1  rounded-md">
        ${resultado.mensagem}
        <small class="flex justify-end text-white">${horaTemp}:${minutoTemp}</small>
        <p>
       </div>`
    );
    chatBotton();
...
function enviar() {
  ...
  mensagemChat.insertAdjacentHTML(
    "beforeend",
    `<div class="flex justify-end  me-5 mt-1 shadow-lg">
...
async function carregarMsg() {
  ...
  try {
    let dados = await fetch(
      `../controllers/listar_registros.php?conversa_id=${conversa_id}&offset=${offset}`
    ); //a var offset vai indicar a quantidadede de registros que já foram carregados
    if (!dados.ok) {
    ...
    if (resposta["status"]) {
      ...
      resposta["dados"].map((item, index) => {
        ...
        if (userAuth === item.user_id) {
          ...
          mensagemChat.insertAdjacentHTML(
            "afterbegin",
            `<div class="flex justify-end me-5 mt-1 items-center gap-1 shadow-lg">
                         <i class="bi bi-trash3-fill text-white opacity-0 cursor-pointer w-5 h-5 rounded-full del"></i>
          ...
          let del_dialog_mensagem = document.getElementById(
            "del_dialog_mensagem"
          );
          ...
          mensagemChat.insertAdjacentHTML(
            "afterbegin",
            `<div  title="${
              item.primeiroNome + " " + item.apelido
            }" class="flex ms-5  gap-2  justify-start">
                    <div class="rounded-full bg-slate-700 shadow-lg w-7 text-white leading-7  h-7 text-[13px] text-center">${
                      item.primeiroNome[0]
                    }</div>
              <p class="flex flex-col max-w-[70%] bg-slate-700  text-justify  text-white justify-center p-2 my-1  rounded-md">
              ${item.conteudo}
              <small class="flex justify-end text-white">${hora}:${minuto}</small>
              <p>
          </div>`
          );
          // chatBotton()
      ...
      mensagemChat.insertAdjacentHTML(
        "afterbegin",
        `<p class="text-yellow-700  text-[12px]">${resposta["msg"]} </p>`
      );
    }
  } catch (error) {
